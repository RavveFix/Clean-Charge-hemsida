# Clean Charge SEO Growth Plan — September 2026

## Mål

Öka Clean Charges organiska synlighet för kommersiella sökningar inom laddboxar, laddinfrastruktur, BRF/fastighet, företag, publik laddning och Monta-drift utan att skapa intern kannibalisering.

Utgångsläge vid granskningen:

- Teknisk SEO-grund finns redan: unika metadata, canonical, sitemap, robots, JSON-LD och SEO-regressionstest.
- Google har börjat crawla/indexera flera nya landningssidor och guider.
- Största möjligheten ligger i tydligare sökintention per URL, lokal relevans, internlänkning, case/E-E-A-T och externa länksignaler.
- Canonical host ska vara `https://www.cleancharge.se`. Apex `cleancharge.se` ska permanent redirectas till www.

## Regler för sökintention

1. En primär kommersiell intention ska ha en tydlig huvudsida.
2. Guider ska stödja, inte konkurrera med, transaktionssidor.
3. Ortsidor ska bara byggas där Clean Charge faktiskt kan leverera och där innehållet kan göras lokalt unikt.
4. Nya sidor ska länka både uppåt till huvudklustret och mot nästa steg i köpresan.
5. Undvik massproducerade stadssidor med endast ortsnamn utbytt.

## Keyword map — 48 prioriterade sökord

| # | Sökord | Intent | Prioritet | Primär URL | Åtgärd |
|---|---|---|---|---|---|
| 1 | laddbox företag | Transactional | P1 | `/foretag` | Förstärk befintlig sida |
| 2 | laddstation företag | Transactional | P1 | `/foretag` | Synonym i H2/body |
| 3 | elbilsladdning företag | Commercial | P1 | `/foretag` | Förstärk copy/internlänkar |
| 4 | laddbox arbetsplats | Transactional | P1 | `/foretag` | Lägg sektion om arbetsplatsladdning |
| 5 | arbetsplatsladdning | Commercial | P1 | `/foretag` | Samma URL, ingen ny sida |
| 6 | laddstolpar företag | Transactional | P1 | `/foretag` | Samma kluster |
| 7 | laddinfrastruktur företag | Commercial | P1 | `/foretag` | Stärk projektering/drift |
| 8 | laddbox företag pris | Commercial | P1 | `/vad-kostar-laddbox` | Internlänk vidare till `/foretag` |
| 9 | laddbox fastighetsbolag | Transactional | P1 | `/fastighetsbolag` | Huvudterm för fastighetsklustret |
| 10 | laddbox hyresfastighet | Transactional | P1 | `/fastighetsbolag` | Lägg tydligare hyresfastighetssektion |
| 11 | laddbox BRF offert | Transactional | P1 | `/fastighetsbolag` | CTA/offerterbjudande |
| 12 | installation laddbox BRF | Transactional | P1 | `/fastighetsbolag` | Projektering + installation |
| 13 | laddbox bostadsrättsförening | Transactional | P1 | `/fastighetsbolag` | Full term i copy |
...
| 22 | laddbox samfällighet | Transactional | P1 | `/samfallighet` | Förstärk befintlig sida |
| 23 | laddstolpar samfällighet | Transactional | P1 | `/samfallighet` | Primär synonym |
| 24 | laddbox samfällighet bidrag | Commercial | P1 | `/samfallighet` | Länk till bidragsguiden |
| 25 | laddbox gemensam parkering | Commercial | P2 | `/samfallighet` | Relevant långsvans |
| 26 | Monta operatör | Commercial | P1 | `/monta` | Metadata förstärkt 2026-09 |
| 27 | Monta laddbox | Commercial | P1 | `/monta` | Huvudkluster |
| 28 | betallösning laddbox | Transactional | P1 | `/monta` | Huvudkluster |
| 29 | debitering laddbox | Commercial | P1 | `/monta` | Huvudkluster |
...
| 39 | DC snabbladdare | Commercial | P1 | `/dc-laddstation` | Synonym och produktintent |
| 40 | snabbladdare företag | Transactional | P1 | `/dc-laddstation` | Företagssektion |
| 41 | DC snabbladdare pris | Commercial | P1 | `/vad-kostar-laddbox` | Prisguide -> DC-sida |
| 42 | laddbox Stockholm BRF | Local transactional | P1 | `/fastighetsbolag/stockholm` | Befintlig ortssida |
| 43 | laddbox BRF Göteborg | Local transactional | P1 | `/fastighetsbolag/goteborg` | Befintlig ortssida |
| 44 | laddbox BRF Malmö | Local transactional | P1 | `/fastighetsbolag/malmo` | Befintlig ortssida |
...
- `/monta` äger software/payment/operator-intent.
- `/publik` äger fysisk publik laddinfrastruktur och kommersiella laddstationer.
- Korslänka, men duplicera inte samma huvudrubriker och FAQ-intentioner.

### `/vad-kostar-laddbox` vs tjänstesidor
...
1. `/fastighetsbolag/uppsala` — om leverans/logistik kan styrkas och lokal efterfrågan är tillräcklig.
2. `/fastighetsbolag/vasteras` — stark geografisk närhet till Örebro och relevant B2B-marknad.
3. `/fastighetsbolag/linkoping` — regional expansion efter data.
...
Prioritera kvalitet före antal ortssidor.

- Behåll Örebro som stark lokal bas.
- Stockholm/Göteborg/Malmö ska ha unik lokal problemformulering och konkret leveransmodell.
...
Mät varje vecka i Google Search Console:

- Impressions per query och landing page.
- Antal queries i position 4–20 — högsta optimeringspotential.
- CTR för sidor i position 1–10.
- Kannibalisering: samma query med flera växlande landningssidor.
- Index Coverage och canonical-val.
- Branded vs non-branded trafik.

Mät varje månad:

- Antal non-brand keywords Top 3 / Top 10 / Top 20.
