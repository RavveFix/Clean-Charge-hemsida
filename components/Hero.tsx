...
const Hero: React.FC = () => {
  ...
  return (
    <section 
      ref={containerRef}
      className="relative min-h-[85vh] md:min-h-screen w-full flex items-center overflow-hidden bg-white pt-24 md:pt-32 lg:pt-40 pb-12 md:pb-16"
    >
      <div className="container mx-auto px-6 relative z-10 w-full max-w-7xl">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-10 lg:gap-8">
          ...
          <div className="w-full lg:w-[55%] flex flex-col justify-center text-center lg:text-left">
            ...
            {/* Tag/Badge */}
            <div className="hero-anim w-fit flex items-center space-x-2 bg-slate-50 text-slate-600 px-4 py-2 rounded-full border border-slate-100 self-center lg:self-start mb-6 md:mb-8 lg:mb-10 transition-colors hover:bg-slate-100 cursor-default">
              <Zap className="w-3.5 h-3.5" />
              <span className="text-[10px] sm:text-xs font-semibold tracking-wide uppercase">Auktoriserad Zaptec &amp; Monta Partner</span>
            </div>

            <h1 className="hero-anim text-4xl sm:text-6xl md:text-7xl lg:text-[5.5rem] font-[800] text-text-primary tracking-tighter leading-[1.05] mb-6 md:mb-8 lg:pr-10">
              Vi levererar,{' '}<br className="hidden sm:block" />
              konfigurerar och{' '}<br className="hidden sm:block" />
              driftar er laddinfrastruktur.
            </h1>
            
            <p className="hero-anim text-lg md:text-xl text-text-primary font-medium leading-relaxed mb-10 md:mb-12 max-w-lg mx-auto lg:mx-0 pr-0 lg:pr-8">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-3 sm:gap-4 hero-anim pt-2 md:pt-4">
              <Link href="/produkter">
                <MagneticButton 
                  strength={15}
                  magneticRadius={60}
