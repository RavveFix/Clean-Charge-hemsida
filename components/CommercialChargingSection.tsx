...
const CommercialChargingSection: React.FC = () => {
    ...
    return (
        <div className="bg-white font-monta animate-in fade-in duration-500">
             {/* Hero Section */}
            <section className="relative pt-28 pb-16 sm:py-24 md:py-32 lg:py-40 bg-slate-900 text-white overflow-hidden min-h-[60vh] sm:min-h-[80vh] flex items-center">
                 {/* Background effects */}
                         ...
                         {commercialProducts.map((product) => (
                            <div key={product.id} id={product.id} className="bg-white rounded-[2rem] sm:rounded-[3.5rem] p-6 sm:p-10 shadow-sm border border-slate-100 flex flex-col md:flex-row gap-6 sm:gap-10 items-center group hover:shadow-2xl hover:border-cc-green/20 transition-all duration-500 scroll-mt-32">
                                <div className="w-full md:w-1/2 aspect-square bg-slate-50 rounded-[2.5rem] p-8 flex items-center justify-center relative overflow-hidden">
                                    ...
                                    <Image src={product.image} alt={`${product.name} – ${product.category === 'snabbladdare' ? 'DC-snabbladdare' : 'AC-laddbox'} för företag`} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-contain relative z-10 group-hover:scale-110 transition-transform duration-500" />
                                </div>
